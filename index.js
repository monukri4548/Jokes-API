...
app.get("/jokes/:id", (req, res) => {
  ...
  const foundId = jokes.find((joke) => joke.id === id);
  res.json(jokes[foundId]);
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  const type = parseInt(req.query.type);
  const foundtypes = jokes.filter((joke) => joke.jokeType === type);
  res.json(foundtypes);
});
...
app.post("/jokes", (req, res) => {
  const newJoke = {
    id: jokes.length + 1,
    jokeText: res.body.text,
    jokeType: res.body.type,
  };
  jokes.push(newJoke);
  res.json(newJoke);
});
...
app.patch("jokes/:id", (req, res) => {
  ...
  replacejoke = {
    id: id,
    jokeText: req.body.text || found.jokeText,
    jokeType: req.body.type || found.jokeType,
  };
  const patchId = found.id;
  jokes[patchId] = replacejoke;
  res.json(replacejoke);
});
